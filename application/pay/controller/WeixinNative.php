...
class WeixinNative extends PayService {
    ...
    public function pay($trade_no, $subject, $totalAmount) {
        ...
        $wxPay = new WxpayNativeService($mchid, $appid, $apiKey);
        $outTradeNo = $trade_no;
        $payAmount = $totalAmount;
        $orderName = $subject;
        $notifyUrl = url("pay/WeixinNative/notify");
        $payTime = time();
        $arr = $wxPay->createJsBizPackage($payAmount, $outTradeNo, $orderName, $notifyUrl, $payTime);

        return $this->qrcode($arr['code_url']);
    }
