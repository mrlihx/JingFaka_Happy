...
class FileService {
    ...
    public static function getUploadQiniuUrl($isClient = true) {
        ...
        switch ($region) {
            ...
            case '东南亚':
                if ($isHttps) {
                    return $isClient ? 'https://upload-as0.qbox.me' : 'https://up-as0.qbox.me';
                }
                return $isClient ? 'http://upload-as0.qiniup.com' : 'http://up-as0.qiniu.com';
            case '华东-浙江2':
                if ($isHttps) {
                    return $isClient ? 'https://upload-cn-east-2.qiniup.com' : 'https://up-cn-east-2.qiniup.com';
                }
                return $isClient ? 'http://upload-cn-east-2.qiniup.com' : 'http://up-cn-east-2.qiniup.com';
            case '华南':
            default:
                if ($isHttps) {
    ...
    public static function getFileName($location, $ext = '', $pre = '') {
        return $pre . join('/', str_split(md5($location), 16)) . random_str(6) . '.' . $ext;
    }
